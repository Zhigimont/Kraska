<?php $country = do_shortcode( '[geoip_detect2 property="country.isoCode"]' ); // variable BY or RU ?>	
	...
	<footer class="wrapper footer-bottom">
		<div class="container">
			<div class="col-xs-8 phone">
				<?php if($country == "RU"): ?>
					<a href="tel:<?=str_replace(" ","",get_field('f_tel_ru','option')); ?>"><img src="<?php bloginfo('template_url'); ?>/img/logo-small.svg" alt="urbanstroi"><?php the_field('f_tel_ru','option') ?></a>
				<?php else: ?>
					<a href="tel:<?=str_replace(" ","",get_field('f_tel','option')); ?>"><img src="<?php bloginfo('template_url'); ?>/img/logo-small.svg" alt="urbanstroi"><?php the_field('f_tel','option') ?></a>
				<?php endif; ?>
			</div>
			<div class="col-xs-4 rec">
				<?php if($country == "RU"): ?>
					<?php the_field('f_rec_ru','option'); ?>
				<?php else: ?>
					<?php the_field('f_rec','option'); ?>
				<?php endif; ?>
				<div class="clearfix"></div>
			</div>
			<div class="col-xs-12 copyright centered">
				Copyright © <a href="http://dasdigital.by" target="_blank">ООО "DIGITAL AUTOMATED SYSTEMS"</a> <?=date('Y'); ?>. Все права защищены
			</div>
		</div>
	</footer>
	...
	<div id="fform" style="display: none;">
		<form id="fformform" class="fform">
			...
			<a href="http://urbanstroi.by/pk/" target="_blank" class="pk-modal">Политика конфиденциальности</a>
		</form>
	</div>
				...
				<div class="row">
					...
					<div class="col-xs-5 right">
						<form id="fformform2" class="fform2">
							...
							<a href="http://urbanstroi.by/pk/" target="_blank" class="pk-modal alignright">Политика конфиденциальности</a>
						</form>
					</div>
				</div>
			</div>
	...
	<?php wp_footer(); ?>
	<?php the_field('metrika_footer','option'); ?>
</body>
</html>
